...
// fieldsOfExpertise, bio, title, website, linkedinUrl, since, groups, team,
// status, profileUrl, avatar. fieldsOfExpertise and groups are
// semicolon-separated within a cell, team is comma-separated. A member is
...
function membersFromCsv(text) {
  ...
  const idx = {
    ...
    fieldsOfExpertise: col('fieldsOfExpertise'), bio: col('bio'), title: col('title'),
    website: col('website'), linkedin: col('linkedinUrl'), since: col('since'), avatar: col('avatar'),
  ...
  return records
    ...
    .map((r) => {
      ...
      return {
        ...
        ...(get(r, idx.linkedin) && { linkedin: get(r, idx.linkedin) }),
