...
test('relatedMembers prioritizes complementary goals over location and similarity', () => {
  const me = { username: 'me', name: 'Me', city: 'Berlin', fieldsOfExpertise: ['Tech'], specialty: ['Python'], aspirations: { field: 'Shëndeti', mentoring: ['Lidhje me një mentor'], business: ['Mundësi pune'] } };
  const all = [
    me,
    ...
    { username: 'hiring', name: 'Hiring', city: 'Roma', aspirations: { business: ['Kandidatë për punësim'] } },
  ];
  ...
  assert.deepEqual(names, ['mentor', 'hiring', 'field', 'city']);
  assert.match(result.find(item => item.member.username === 'mentor').reason, /Mentor në Shëndeti/);
  assert.match(result.find(item => item.member.username === 'hiring').reason, /kërkon kandidatë/);
...
test('relatedMembers connects people offering help with people seeking it', () => {
  const mentor = { username: 'mentor', name: 'Mentor', aspirations: { mentoring: ['Mundësi për të mentoruar'], business: ['Kandidatë për punësim', 'Mundësi investimi'] } };
  const result = relatedMembers(mentor, [
    mentor,
    { username: 'mentee', name: 'Mentee', aspirations: { mentoring: ['Lidhje me një mentor'] } },
    { username: 'candidate', name: 'Candidate', aspirations: { business: ['Mundësi pune'] } },
    { username: 'founder', name: 'Founder', aspirations: { business: ['Financim për projekt / biznes'] } },
  ], 10);
  assert.deepEqual(result.map(item => item.member.username), ['candidate', 'founder', 'mentee']);
  assert.match(result.find(item => item.member.username === 'mentee').reason, /Kërkon mentorim/);
  assert.match(result.find(item => item.member.username === 'candidate').reason, /mundësi pune/);
  assert.match(result.find(item => item.member.username === 'founder').reason, /financim/);
});

test('relatedMembers suggests peers pursuing the same target field or specialization', () => {
  const me = { username: 'me', name: 'Me', aspirations: { field: 'Teknologji', subfield: 'Siguri kibernetike' } };
  const result = relatedMembers(me, [
    me,
    { username: 'same-target', name: 'Same target', aspirations: { field: 'teknologji', subfield: 'siguri kibernetike' } },
    { username: 'same-field', name: 'Same field', aspirations: { field: 'Teknologji' } },
  ], 10);
  assert.deepEqual(result.map(item => item.member.username), ['same-target', 'same-field']);
  assert.match(result[0].reason, /Siguri kibernetike/i);
});
